...
const defaultConfig = const_getListConfig('list', 'ZlistPanel');
...
class ZlistPanel extends React.PureComponent {
    static propTypes = {
        ...
        btnKeySorterMap: PropTypes.object,
    };
    ...
    componentDidMount() {
        ...
        this.methods.onSearch(this.props.searchForm?.values);
    ...
    componentDidUpdate(prevProps, prevState) {
        if (
            ...
            this.props.tableParams !== prevProps.tableParams ||
    ...
    methods = {
        ...
        // 获取列表数据
        /**
         * (isMergeData, otherQuery)传参方式是兼容版本方式，
         * 现在传参方式：
         * ({
         *  merge:false,//是否请求的列表数据合并到现有的列表
         *  query:{},//其他的请求接口的参数
         *  clearSearchForm:false,//是否清空当前的查询列表
         * })
         */
        getListData: (isMergeData, otherQuery) => {
            let merge = isMergeData;
            let moreQuery = otherQuery;
            let opt = {};
            if (dataType.isObject(isMergeData)) {
                opt = isMergeData;
                merge = isMergeData.merge;
                moreQuery = isMergeData.query;
            }
            const clearSearchForm = opt.clearSearchForm || (moreQuery || {}).clearSearchForm;
            let querys = {};
            if (!clearSearchForm) {
                querys = this.searchQuery || {};
            }
            if (dataType.isObject(moreQuery)) {
                this.page.pageNumber = moreQuery.pageNumber || this.page.pageNumber;
                this.page.pageSize = moreQuery.pageSize || this.page.pageSize;
                this.moreQuery = { ...this.moreQuery, ...moreQuery };
                querys = Object.assign({}, querys, moreQuery);
            }
            if (clearSearchForm && this.ZsearchFormMethods) {
                this.ZsearchFormMethods.searchReset();
                return;
            }
                ...
                apiPromise
                    .then((re) => {
                        ...
                        if (Array.isArray(data)) {
                            ...
                            if (this.page.pageNumber > 1 && !list.length) {
                                //没有数据时，降低页数再次请求
                                this.page.pageNumber--;
                                return this.methods.getListData();
                            }
                        }
        ...
        findData: (value, list, other) => {
            ...
            key = key || 'id';
            list = list || this.methods.currentListData();
            let newData = null;
            let hasRemove = false;
            ...
            return hasRemove || newData;
        },
        addChildrenData: (childs, value) => {
            const list = this.methods.currentListData();
            const data = this.methods.findData(value, list);
            if (data) {
        ...
        removeOneData: (row) => {
            ...
            key = key || 'id';
            const list = this.methods.currentListData();
            if (this.props.paginationType === 'infinite') {
                this.setState({
            ...
            ? [
                  {
                      ...
                      render: (text, record, index) => {
                          ...
                          const detailBtnName = this.props.detailBtnName || '详情';
                          const detailBtn =
                              // <span key="detail">
                              this.getDiffBtn(
                                  'default',
                          ...
                          // </span>
                          const updateBtnName = this.props.updateBtnName || '修改';
                          const updateBtn =
                              // <span key="update">
                              this.getDiffBtn(
                                  'default',
                          ...
                          // </span>
                          const deleteBtnName = this.props.deleteBtnName || '删除';
                          const deleteBtn =
                              // <span key="delete">
                              this.getDiffBtn(
                                  'danger',
                          ...
                          const moreBtnName = (
                              <span>
                                  <span>{this.props.moreBtnName || '更多'}</span>
                                  <i
                          ...
                          if (Array.isArray(customBtns)) {
                              btns = btns.concat(customBtns);
                          }
                          const btnKeySorterMap = {
                              detailBtn: 1,
                              updateBtn: 2,
                              deleteBtn: 3,
                              moreBtn: 4,
                              ...(this.props.btnKeySorterMap || {}),
                          };
                          btns = btns.sort((prev, next) => {
                              if (btnKeySorterMap[prev.key] && btnKeySorterMap[next.key]) {
                                  return btnKeySorterMap[prev.key] - btnKeySorterMap[next.key];
                              }
                              return 0;
                          });
    ...
    getSearchFormMethods = (form, methods) => (this.ZsearchFormMethods = methods);
}
ZlistPanel.prototype.getPanleHeader = function() {
    return const_getPanleHeader.call(this, true);
};
export default ZerodRootContext.setConsumer(
    ZerodMainContext.setConsumer(ZerodLayerContext.setConsumer(withRouter(ZlistPanel))),
);
