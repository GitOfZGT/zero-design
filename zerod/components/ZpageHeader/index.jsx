...
export class ZpageHeader extends React.Component {
    ...
    componentDidMount() {
        ...
        if (this.props.ceiling && this.pageHeaderBox) {
            this.setState({
                pageHeaderBox: this.pageHeaderBox,
            });
        }
    }
    // componentDidActivate() {
    //     if (this.pageHeaderBox && this.props.ceiling) {
    //         this.setState({ pageHeaderBox: this.pageHeaderBox });
    //     }
    // }
    // componentWillUnactivate() {
    //     if (this.pageHeaderBox && this.props.ceiling) {
    //         this.setState({ pageHeaderBox: null });
    //     }
    // }
    render() {
        const {
        ...
        const mb = 'bottom-10';
        const comtents = (
            <section className={`z-page-header ${className || ''}`} {...others} ref={(el) => (this.domWrapperEl = el)}>
                            ...
                            <div className="z-flex-1">
                                ...
                                {content ? (
                                    typeof content === 'function' ? (
                                        ...
                                        <p className={`z-margin-bottom-20 z-text-gray`}>{content}</p>
                                    )
                                ) : null}
                                {/* {title || content ? (
        ...
        return this.state.pageHeaderBox && ceiling
            ? ReactDOM.createPortal(comtents, this.state.pageHeaderBox)
            : comtents;
    }
}
