...
class TreeTitle extends React.PureComponent {
    ...
    moreMenu = (record, index) => {
        ...
        return hasItems ? this.props.moreBtnType == 'rounding' ? items : <Menu onClick={onClick}>{items}</Menu> : null;
    };
    methods = {
    ...
    getBtn = (type, btnName, click, show, disabled, opt = {}) => {
        return show ? (
            <Button onClick={click} size="small" type={type} disabled={disabled} {...opt}>
                {btnName}
            </Button>
        ) : null;
        // return show ? (
    ...
    componentDidMount() {
        this.nodeRef.current.firstElementChild.style.maxWidth =
            this.nodeRef.current.clientWidth - this.nodeRef.current.querySelector('.z-tree-btns').clientWidth + 'px';
    }
    render() {
        const tool = this.props.tool;
        const {
        ...
        const moreBtn =
            typeof moreBtnRender === 'function' ? (
                moreBtnRender(record, index, tool)
            ) : (
                <Button
                    size="small"
                    onClick={(e) => {
        ...
        let moreBtnRendering = null;
        if (this.hasMoreMenu) {
            const menus = this.moreMenu(record, index);
            if (menus) {
                moreBtnRendering =
                    this.props.moreBtnType == 'rounding' ? (
                        <ZroundingButton items={menus} onVisibleChange={this.onVisibleChange}>
                            {moreBtn}
                        </ZroundingButton>
                    ) : (
                        <Dropdown
        ...
        return (
            <span className="z-flex-space-between" ref={this.nodeRef}>
                <span className="z-tree-title" title={React.isValidElement(this.props.name) ? '' : this.props.name}>
                    {this.props.name}
                </span>
