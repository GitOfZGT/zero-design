...
export function parseJsonToObject(frontDef) {
    // console.log(frontDef);
    let pare = {};
    if (
        frontDef &&
        typeof frontDef === 'string' &&
        /^\{(.|\n)*\}$/.test(frontDef.replace(/(^\s+|\s+$)/g, '').replace(/\\(?=")/g, ''))
    ) {
        pare = JSON.parse(frontDef);
    } else if (frontDef && typeof frontDef === 'object') {
        pare = frontDef;
    }
    return pare;
}
